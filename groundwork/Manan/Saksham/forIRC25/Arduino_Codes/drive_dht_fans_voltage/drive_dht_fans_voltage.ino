...
std_msgs::Float32MultiArray volt_Current;
ros::Publisher volt_curr_data("volt_current", &volt_Current);
...
{ 
  ...
  nh.advertise(volt_curr_data);
  temp_c.data_length = 3;
  volt_Current.data_length = 2;
  pinMode(fans,OUTPUT);
  digitalWrite(fans,LOW);
}
...
{
  float data2[2] = {0 , 0};
  data2[0] = ina219.getBusVoltage_V(); // Voltage in volts
  data2[1] = ina219.getCurrent_mA();
  ...
  volt_Current.data = data2;
  volt_curr_data.publish(&volt_Current);
  pub1.publish(&vels);
  nh.spinOnce();
}